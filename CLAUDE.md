...
1. **데이터 소스** — `UsageDataReader.readAll()`이 `~/.claude/projects/`를 재귀 순회하며 `.jsonl`을 읽는다. `type=="assistant"` 라인만 파싱하고(단, `SYNTHETIC_MODEL_NAMES` = rate-limit 거부 응답의 `<synthetic>`은 실사용이 아니므로 제외), `dedupeKey`(`message.id` 우선, 없으면 `uuid`)로 중복 제거하며, ISO8601(소수초 유무 둘 다) 타임스탬프를 처리한다. 외부 전송 없는 완전 오프라인.
...
- **리더의 메모리는 "얼마나 보관하느냐"가 아니라 "얼마나 할당했다 놓느냐"가 결정한다.** 실측(1,416파일·897MB·46K엔트리): 살아 있는 데이터는 `malloc in_use` 기준 **24MB**뿐인데 프로세스 footprint는 **2.4GB 피크 / 638MB 잔여**였다. 자료구조를 전부 비워도(`cachedEntries`·`fileCache` 모두 `removeAll`) 잔여가 안 줄고 `malloc_zone_pressure_relief`도 듣지 않는다 — 24MB가 500MB 넘는 영역에 흩뿌려진 **단편화**이기 때문이다. 그래서 **엔트리 pruning은 무의미하다**(그 24MB 중 일부를 줄일 뿐 하이워터마크는 그대로). 범인은 두 가지였고 둘 다 제거했다:
  - ① 파일마다 `readToEnd()`로 통짜 버퍼를 할당 — JSON 파싱을 전부 건너뛰고 **읽어서 줄만 나눠도** footprint가 730MB였다(파싱 몫은 30MB에 불과). 지금은 **인스턴스 하나짜리 재사용 버퍼**(`readBuffer`, 1MiB)에 `read(2)`로 담고 오프셋을 이어 붙인다. 청크를 `Data`로 이어 붙이는 방식은 재할당이 단편화를 더 키워(885MB) 폐기했고, 메모리 매핑은 39MB로 효과는 좋았지만 **트렁케이트 시 SIGBUS** 위험 때문에 채택하지 않았다.
  - ② `JSONSerialization`의 오토릴리스 객체 그래프가 `readAll()` 한 번이 끝날 때까지 누적 — GCD는 워크아이템이 끝나야 풀을 비운다. 지금은 **줄 단위 `autoreleasepool`**로 상한을 한 줄치로 낮췄다.
  - 결과: 피크 **2,374 → 44MB**, 콜드 파싱 **22~38초 → 8~9초**(편차도 사라짐). 집계 숫자는 실데이터 47,065엔트리에서 비트 단위로 동일.
  - 부수적으로 `type=="assistant"` **바이트 프리필터**(`memmem`)가 33.2만 줄 중 70%를 JSON 파싱 이전에 걷어내고, 브리지된 NSString이 파싱 버퍼를 붙잡지 못하도록 `model`만 `detachedCopy`로 떼어낸다. `UsageEntry.sessionId`/`uuid`는 읽는 곳이 0건이라 제거했다(다시 넣기 전에 "읽는 곳이 있는가"부터 확인할 것).
- **`readAll()`의 파일 캐시는 "성공적으로 소비한 크기"만 기록한다**: `FileCacheState.size`를 갱신하는 순간 그 파일은 다음 사이클의 `cached.size == size` 스킵에 걸리므로, 읽기에 실패한 채로 크기를 기록하면 미읽은 구간이 사실상 유실된다(그 파일이 다시 자라거나 앱을 재시작하기 전까지 회수 불가). 그래서 `entries(in:)`은 `FileReadOutcome.consumed`/`.notConsumed`로 성공/실패를 구분하고, `.notConsumed`일 땐 크기도 `didChange`도 건드리지 않는다. **`didChange`를 건드리지 않는 게 특히 중요하다** — 영구 실패 파일 하나가 매 사이클 `didChange = true`를 세우면 라이프타임 엔트리 O(N log N) 정렬/재병합 스킵 최적화가 통째로 무효화된다. 재시도는 `retryDelay()` 지수 백오프(15초→최대 10분, `maxReadFailures`회 뒤 포기하고 크기를 받아들임)로 제한하되, **관측 크기가 달라지면 백오프를 무시하고 즉시 재시도**한다 — 그러지 않으면 쓰는 중이라 개행이 아직 없는(=`.notConsumed`) 활성 세션 파일이 백오프에 걸려 사용량 반영이 몇 분씩 늦어진다.
- **전역 병합은 `fileCache.keys.sorted()` 순서로 돈다**(`.values` 순회 금지). `mergeUsageEntriesByMaxTokens`가 토큰만 `max`로 병합하고 timestamp/model은 먼저 본 쪽을 남기므로, Swift Dictionary의 프로세스별 랜덤 해시 시드가 결과에 새어 나올 수 있다. 실측상 지금은 여러 파일에 걸친 중복 450건이 모두 동일한 timestamp를 기여해 관측 가능한 차이가 없지만(즉 이건 잠재 결함 제거), 그 전제가 깨지는 순간 조용히 실행마다 다른 값이 나온다.
- **dedupe 키에 `requestId`를 다시 섞지 말 것**: 같은 `message.id`가 `requestId` 유무에 따라 두 키로 갈라져 한 턴이 이중 집계된다. 실데이터에서 한 `message.id`가 서로 다른 `requestId`를 갖는 경우는 0건이라 변별력도 없다.
