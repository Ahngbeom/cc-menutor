...
- **릴리스(`gh release create`) 시 GitHub Actions(`.github/workflows/bump-homebrew-formula.yml`)가 `Ahngbeom/homebrew-tap`의 `Formula/cc-menutor.rb`를 자동 갱신한다**(`mislav/bump-homebrew-formula-action`, 커밋 SHA로 고정). 다른 저장소에 쓰기 때문에 `HOMEBREW_TAP_TOKEN`(**classic PAT**, `repo`+`workflow` 스코프) 시크릿이 필요하다 — **fine-grained PAT은 이 액션에서 공식 지원되지 않고 `Error: unexpected HTTP 303 response`로 실패한다**(v1.4 릴리스 때 실제로 겪은 문제; 액션 공식 README와 유사 리다이렉트 실패 사례로 확인). 시크릿이 만료·누락되거나 fine-grained로 잘못 발급되면 이 단계만 조용히 실패할 수 있으니 릴리스 후 `gh run list --repo Ahngbeom/cc-menutor --workflow "Update Homebrew formula"`로 확인할 것.
