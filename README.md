...
<p align="center">
  ...
  <sub>「숫자 색 표시」를 켰을 때 사용 비율에 따라 바뀌는 메뉴바 색 — 앱 코드로 렌더링한 예시(값은 가상)</sub>
</p>
...
- **메뉴바 동적 색상(선택)** — 켜면 항목 계열별(한계·오늘·누적) 색상 안에서 사용 비율만큼 색이 짙어짐, 모델명은 모델별 고정색. 기본은 시스템 기본색([아래](#메뉴바-동적-색상) 참고)
...
- **무드 아이콘** — 고정 아이콘 대신 **블록 숫자와 같은 사용 비율**(서버 실측 5시간 % → 예산 대비 → 블록 경과율)에 따라 모양·색이 바뀌는 아이콘을 메뉴바에 표시. 색은 옆의 블록 토큰·비용과 같은 빨강 계열로 짙어져, 같은 비율이면 아이콘과 숫자가 같은 색입니다. statusLine 연동을 해 두면 시간이 흘렀다는 이유만으로 「한계 근접」이 뜨지 않고, 초반에 빨리 태우면 곧바로 「가속」이 됩니다. 5단계(대기/여유/몰입/가속/한계 근접)마다 형태가 뚜렷이 달라지고(예: 불꽃이 불씨→작은 불꽃→큰 불꽃→화염으로 자람), 단계별 애니메이션(불꽃 일렁임, 화산 연기 피어오름, 게이지 바늘 떨림, 온도계 기포 등)이 함께 커집니다. "🎨 무드 아이콘 모양" 서브메뉴에서 불꽃(기본)/온도계/배터리/게이지/화산 5종 중 하나를 고를 수 있습니다.
...
<img src="docs/images/mood-themes.png" alt="무드 아이콘 5종 테마 × 5단계 (라이트/다크)" width="302">

<sub>테마별 5단계(대기·여유·몰입·가속·한계 근접) — 앱 코드로 렌더링한 예시. 단계는 모양으로도 구분되므로 색 없이도 읽힙니다.</sub>
...
|------|-------|--------|------------|------------------|
| Claude Fable / Mythos 5.1 | $10/M | $50/M | **$0.25/M** | $12.50/M |
...
| Claude Sonnet 5 | $2/M | $10/M | $0.20/M | $2.50/M |
| Claude Sonnet 4.x · 3.x | $3/M | $15/M | $0.30/M | $3.75/M |
| Claude Haiku 4.5 | $1/M | $5/M | $0.10/M | $1.25/M |
| Claude Haiku 3.5 (은퇴) | $0.80/M | $4/M | $0.08/M | $1.00/M |

캐시 쓰기 단가는 공식 배수(5분 = input×1.25, 1시간 = ×2.0)로 input에서 파생합니다. **캐시 읽기는 모델마다
배수가 다릅니다** — 대부분 input×0.1이지만 Opus 5.5는 ×0.05, Fable·Mythos 5.1은 ×0.025입니다(굵게 표시).
Claude Code 사용량은 대부분 캐시 읽기라서 이 배수가 추정 비용을 좌우합니다. 테이블에 없는 모델은 같은
family의 **최신 버전 단가**로 추정하고 드롭다운에 `⚠ 미상 모델` 로 고지합니다.
...
- **⌨ 메뉴바 표시 항목** — 표시할 필드 선택·순서, 맨 아래 **「숫자 색 표시」**(기본 꺼짐)
...
**기본은 꺼져 있습니다.** 메뉴바 글자는 시스템 기본색으로 표시되고, 상태는 무드 아이콘과 드롭다운이 색으로 보여 줍니다.
`⌨ 메뉴바 표시 항목` 맨 아래 **「숫자 색 표시」**를 켜면 아래 방식으로 숫자 자체에 색이 들어갑니다.

> **왜 기본이 꺼짐인가** — macOS 26(Tahoe)의 메뉴바는 배경화면이 비치고, 「메뉴 막대 배경 보기」를 켜도
> 색이 배경화면에서 옵니다. 실측해 보니 중간 밝기·채도 높은 배경화면에서 색 글자의 명암비가 **1.2~2.3:1**까지
> 떨어졌습니다(같은 조건에서 시스템 글자는 3.6~7.7:1). 시스템 기본색은 메뉴바가 배경에 맞춰 보정해 주므로 늘
> 시스템 글자만큼 읽힙니다. 메뉴바가 불투명한 환경이거나 색을 선호하면 켜서 쓰세요.
> 자세한 측정은 [#19](https://github.com/Ahngbeom/cc-menutor/issues/19) 참고.

켜면 메뉴바의 각 항목은 **수치가 얼마나 찼는지**에 따라 색이 바뀝니다.
...
예산 경고가 뜨면, 색을 켠 경우 타이틀 전체가 주황/빨강으로 덮이고 `⚠N%`가 붙습니다. 색을 끈 경우에는 글자는 그대로
두고 굵게 바꾼 뒤 `⚠️N%`를 붙입니다(이모지는 배경과 무관하게 보이도록 자체 색을 가집니다).
