...
```bash
curl -fsSL https://raw.githubusercontent.com/Ahngbeom/cc-menutor/main/scripts/install.sh | CC_MENUTOR_VERSION=1.12 bash
```
...
- **메뉴바 동적 색상** — 각 항목의 색이 사용 비율에 따라 초록→노랑→주황→빨강으로 자동 전환, 모델명은 모델별 고정색([아래](#메뉴바-동적-색상) 참고)
...
메뉴바의 각 항목은 직접 고른 색 대신 **수치가 얼마나 찼는지**에 따라 색이 연속으로 바뀝니다
(0% 초록 → 노랑 → 주황 → 100% 빨강). 항목마다 "얼마나 찼는지"의 기준은 다릅니다:

| 항목 | 기준 |
|---|---|
| 블록 토큰 · 블록 출력 토큰 · 블록 비용 | 서버 실측 5시간 사용률(statusLine 연동 시) → 예산 대비 비율(`CLAUDE_MONITOR_TOKEN_BUDGET`/`COST_BUDGET` 설정 시) → 5시간 블록 경과율 |
| 남은 시간 | 5시간 블록 경과율 |
| 오늘 토큰 · 오늘 비용 | 개인 최고 일일 기록 대비 (기록이 없으면 기본색) |
| 누적 토큰 | 다음 마일스톤(1M·10M·100M·1B, 이후 ×10)까지 진행률 |
