...
- **메뉴바**는 선택한 통화 하나로만 표시됩니다 — `⌨ 12.3K ₩6,623`
- **드롭다운의 주요 합계 3곳**(현재 5시간 블록·오늘·전체 누적)은 양쪽을 병기합니다 —
  `예상 비용: $4.56 (₩6,623)`. CLI 실측 달러 금액을 계속 대조할 수 있게 하려는 것입니다.
  모델별 분해·소모율·기록 섹션은 달러로 유지됩니다(행이 지나치게 길어지는 것을 막기 위함).
- 드롭다운 하단에 적용된 환율과 기준일이 표시됩니다 — `💱 환율 ₩1,452.35/$1 · 2026-07-29 기준`
...
| 마지막 성공 조회가 24시간 초과 | 병기는 유지하고 `⚠ 환율 오래됨 (기준일) — 네트워크 확인` 배너 표시 |
